...
// This workaround can be removed when db::Env::DeleteFile is removed.
#if defined(_WIN32)
...
public:
  ...
  //
  // The result of Default() belongs to db and must never be deleted.
  static Env* Default();
  ...
  // new file. On success, stores a pointer to the new file in *result
  // and returns OK. On failure stores nullptr in *result and returns
  // non-OK.
  //
  // The returned file will only be accessed by one thread at a time.
  ...
  // The returned file will only be accessed by one thread at a time.
  //
  // May return a NotSupported status if this Env does not allow
  // appending to an existing file. Users of Env (including the db
  // implementation) must be prepared to deal with an Env that does
  ...
  //
  // A future release will remove DeleteFile and the default implementation of
  // RemoveFile.
